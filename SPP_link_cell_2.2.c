...
os: Windows 7/10
Compiler: MinGW gcc
Compile command: gcc SPP_link_cell_2.2.c SPP_force.c SPP_sub.c -o SPP -std=c99

os: Ubuntu
Compiler:gcc
Compile command: gcc SPP_link_cell_2.2.c SPP_force.c SPP_sub.c -o SPP -std=gnu99 -lm
...
void compF_Intr(Particle **order, int N, real *l, long *seed){
...
void compF_LC(Cell *grid, int *nc, real *l, real r_cut, int PBC_state) {
            ...
            for (ParticleList *i=grid[index(ic,nc)]; NULL!=i; i=i->next) {
				...
				if(PBC_state)
					for (int d=0; d<DIM; d++){
						flag_wall=0;
						if(ic[d]==(nc[d]-1)){
							wall_particle.x[d] = 2*l[0]-(i->p.x[d]);
							r_wall = l[0]-(i->p.x[d]);
							flag_wall=1;
						}
						if(ic[d]==0){
...
void timeIntegration_LC(real t, real delta_t, real t_end, Cell* grid, int *nc, real* l, real r_cut, int N, Particle **order) {
	...
	while (t < t_end) {
		t += delta_t;
		compF_LC(grid,nc,l,r_cut,PBC_state);
		compF_Intr(order,N,l,seed);
